...
function State(str, regExpStr, traces, multiline) {
    this.str = str;
...
State.prototype.recordMatch = function(idx, from, to) {
    var match;
    match = this.str.substring(from, to);
        ...
        switch (node.type) {
            ...
            case Node.REPEAT:
                ...
                if (lastIdx !== undefined && lastIdx === state.idx) {
                    ...
                    if (node.min === 0 && 'firstMatchIdx' in node.parseEntry) {
                        var firstMatchIdx = node.parseEntry.firstMatchIdx;
                        var lastMatchIdx = node.parseEntry.lastMatchIdx;
                        for (var matchIdx = firstMatchIdx;
                                matchIdx <= lastMatchIdx; matchIdx++)
                        {
                            state.resetMatch(matchIdx);
                        }
                    }
...
function exec(matchStr, startNode, lastIndex, multiline) {
    idCounterTrace = 0;
    ...
    while (lastIndex < matchStr.length) {
        state = new State(matchStr, regExpStr, traces, multiline);
        state.idx = lastIndex;
