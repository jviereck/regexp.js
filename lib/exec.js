...
function match(state, node) {
    function fork(parentNode, childNode, resetMatches) {
        var traceLastItem = state.trace.lastItem;
        ...
        if (!res && state.trace.lastItem !== traceLastItem) {
            // If the trace failed, then recover the last item.
            state.trace.lastItem = traceLastItem;
        }
        return res;
    }
